...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    await guild.bans.fetch().then(async (bans) => {
      ...
      let bannedID = bans.find((ban) => ban.user.id == unbanUser);
      ...
      await guild.bans.remove(unbanUser, reason).catch((err) => {
    ...
    const ubEmbed = new EmbedBuilder()
      .setColor("Green")
      .setDescription(
        `***:white_check_mark: Successfully Unbanned ${unbanUser.username} || reason: ${reason}***`
      );
