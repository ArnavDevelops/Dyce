  ...
  async execute(oldChannel: any, newChannel: any) {
    try {
      ...
      if (oldChannel.permissionOverwrites !== newChannel.permissionOverwrites) {
        ...
        newChannel.permissionOverwrites.cache.forEach(async (newOverwrite: any) => {
          if (newOverwrite.type === 0) {
            const oldOverwrite = oldChannel.permissionOverwrites.cache.get(newOverwrite.id);

            const newlyGrantedPermissions = newOverwrite.allow.toArray().filter((perm: any) => !oldOverwrite.allow.has(perm));
            const newlyDeniedPermissions = newOverwrite.deny.toArray().filter((perm: any) => !oldOverwrite.deny.has(perm));
            const defaultPermissions = oldOverwrite.allow.toArray().filter((perm: any) => !newOverwrite.allow.has(perm) && !newOverwrite.deny.has(perm))
              .concat(oldOverwrite.deny.toArray().filter((perm: any) => !newOverwrite.deny.has(perm) && !newOverwrite.allow.has(perm)));
