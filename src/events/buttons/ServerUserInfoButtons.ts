    ...
    if (interaction.isButton()) {
      ...
      //User info's button
        ...
        const memberRoles = targetMember.roles.cache
          ...
          .map((r: any) => r.toString())
          .join(`\n`);
        const memberRolesSize = targetMember.roles.cache.filter((r: any) => r.id !== interaction.guild.id).size
        ...
        await Promise.all(user.flags.toArray().map(async (badge: any) => {
        ...
        try {
          ...
          if (data.public_flags_array) {
            ...
            await Promise.all(data.public_flags_array.map(async (badge: any) => {
